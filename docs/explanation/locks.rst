...
This section outlines various locking operations available for the spinlocks previously mentioned, along with the appropriate contexts for their use.

spin_lock()
~~~~~~~~~~~

`spin_lock()`_ is the simplest and fastest variation of spinlocks operations.
It can be used to prevent other CPUs from entering the critical region, even when locking from hardware interrupts (IRQs).
Since hardware IRQs have higher priority than softIRQs (`software interrupts`_) and `tasklets`_, this lock variation is advantageous for its speed.
The counterpart to this lock is `spin_unlock()`_, which releases the lock.

spin_lock_bh()
~~~~~~~~~~~~~~

On `spin_lock_bh()`_, the suffix ``_bh()`` refers to "Bottom Halves", historically the old term for softIRQs.
This lock operation is used to synchronize between **user context** and softIRQs, preventing the current user context from being interrupted by a softIRQ and blocking access to the critical region by another CPU running the same softIRQ.
The counterpart to this lock is `spin_unlock_bh()`_, which releases the lock.

.. note::

  **User context**: refers to the execution context of a process that is currently executing a `system call`_ on behalf of a **user-space** application.
  When the kernel is running in user context, it is processing a request initiated by a user-space application and has access to the full range of kernel services, including the ability to sleep (i.e., block) if necessary.


spin_lock_irq()
~~~~~~~~~~~~~~~

This variation is useful for locking between `hardware interrupts`_ and softIRQs.
When a hardware IRQ handler shares data with a softIRQ, there are two main concerns: a softIRQ can be interrupted by a hardware interrupt, and the critical region might be entered by a hardware interrupt on another CPU.
`spin_lock_irq()`_ can be used in softIRQ routines to address this issue.
For hardware IRQ handlers, this lock is unnecessary since softIRQs, having lower priority, cannot interrupt hardware IRQs.
The counterpart to this lock is `spin_unlock_irq()`_, which releases the lock.


spin_lock_irqsave()
~~~~~~~~~~~~~~~~~~~

This variant saves the interrupt state (whether interrupts were enabled or disabled) in a flags variable, which is then passed to `spin_unlock_irqrestore()`_.
This allows the same code to be used inside a hardware IRQ handler (where interrupts are already off) and in softIRQs (where interrupt disabling is required).
Note that softIRQs (including tasklets and timers) run on return from hardware interrupts, so `spin_lock_irq()`_ also prevents these from executing.
In this sense, `spin_lock_irqsave()`_ is the most general and powerful locking function.
This locking mechanism is also used when data needs to be shared between two hardware IRQ handlers, a rare but possible scenario.
This lock is essentially a superset of all the previous spinlock primitives.
The counterpart to this lock is `spin_unlock_irqrestore()`_, which releases the lock and clean the flags variable.

Sleeping Locks (Blocking locks)
-------------------------------

Sleeping locks, also known as blocking locks, have the primary characteristic that tasks holding them can be put to sleep.
Unlike spinlocks, sleeping locks are used in kernel regions where the process context can sleep, typically in user contexts.
Most sleeping locks in the Linux kernel are mutexes, a specific type of locking primitive designed to enforce serialization on shared memory systems.
